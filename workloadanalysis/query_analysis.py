...
def analyze_sqlshare(database, all_owners = True):
    ...
    for owner in owners:
        if owner == '':
            ...
            queries_q = 'SELECT query, plan, expanded_plan_ops_logical, expanded_plan_ops, ref_views from sqlshare_logs where has_plan = true group by query, plan, expanded_plan_ops_logical, expanded_plan_ops, ref_views order by to_timestamp(time_start, \'MM/DD/YYYY HH12:MI:SS am\')'
            ...
            queries_q = 'SELECT query, plan, expanded_plan_ops_logical, expanded_plan_ops, ref_views from sqlshare_logs where has_plan = true and '+ owner_condition +'  group by query, plan, expanded_plan_ops_logical, expanded_plan_ops, ref_views order by to_timestamp(time_start, \'MM/DD/YYYY HH12:MI:SS am\')'
