...
def getmetrics(database):
    ...
    f.write(
        "id, length, expanded_length, runtime, tables, columns, distinct_columns, expressions, log_ops, distinct_expr, distinct_ops, ref_views\n")
    f2.write(
        "id, length, expanded_length, runtime, tables, columns, distinct_columns, expressions, log_ops, distinct_expr, distinct_ops, ref_views, query\n")
    for i, q in enumerate(queries):
        ...
        q['id'], len(q['query']), len(expanded_query), q['runtime'], q['tables'], q['columns'], q['dis_columns'],
        q['expressions'], q['log_ops'], q['dis_expr'], q['dis_log_ops'], len(set(total_ref_views)),
        q['query'].replace(',', '!')))
