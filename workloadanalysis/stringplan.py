...
def stringify_plan(database, mode=1):
    ...
    plans = list(db.query("select min(id) as id, query, plan from sqlshare_logs where has_plan = true group by query, plan order by id"))
    out_file = open('/Users/shrainik/Dropbox/Research/SQLShare/query-reco/data/for_q_predictions/query_plans_parsed.txt', 'w')
    out_file_q = open('/Users/shrainik/Dropbox/Research/SQLShare/query-reco/data/for_q_predictions/orig_queries-do-not-use.txt', 'w')
